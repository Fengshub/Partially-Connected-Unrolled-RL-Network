...
class RegularizerNetwork_B(Model):
    def __init__(self, voxel_shape):
        ...
        self.conv1 = tf.keras.layers.Conv3D(4, 11, padding='same', kernel_initializer=tf.keras.initializers.lecun_normal(seed=42), activation='tanh')
        self.conv2 = tf.keras.layers.Conv3D(4, 7, padding='same', kernel_initializer=tf.keras.initializers.lecun_normal(seed=42), activation='tanh')
        self.conv3 = tf.keras.layers.Conv3D(1, 3, kernel_initializer=tf.keras.initializers.Zeros(), padding='same', activation='tanh')
    ...
    def call(self, x):
        ...
        # x=tf.keras.layers.ZeroPadding3D(padding=((of,of),(of,of),(5,5)))(x)
        ...
        x = self.conv2(x)
        x = self.conv3(x)
        # x = tf.keras.layers.Cropping3D(cropping=((of, of), (of, of), (5,5)))(x) 
...
class RegularizerNetwork_F(Model):
    def __init__(self, image_shape):
        ...
        self.conv1 = tf.keras.layers.Conv2D(4, 11, padding='same', kernel_initializer=tf.keras.initializers.lecun_normal(seed=42), activation='tanh')
        self.conv2 = tf.keras.layers.Conv2D(4, 7, padding='same', kernel_initializer=tf.keras.initializers.lecun_normal(seed=42), activation='tanh')
        self.conv3 = tf.keras.layers.Conv2D(1, 3, kernel_initializer=tf.keras.initializers.Zeros(), padding='same', activation='tanh')
        self.flatten_layer = tf.keras.layers.Flatten()
...
mat = sio.loadmat('data_MLA_index.mat')
...
early_stop = EarlyStopping(monitor='output_2_loss',patience=2,verbose=1,restore_best_weights=True,min_delta=1e-4)
...
# %%
history=model.fit(X_train[0:Y_train.shape[0]-doff], [X_train[0:Y_train.shape[0]-doff],Y_train[0:Y_train.shape[0]-doff]], epochs=20, batch_size=1, verbose=1, shuffle=True, callbacks=[early_stop])
# %% --- Model testing ---
doff = 8
...
for pid in range(pid_base,pid_base+doff):
...
# savemat('output_LiLoR_MLA.mat',{'outputX':generated_images,'outputY':generated_objects,'history':history.history})
# %% # --- Plot test outputs ---
